...
const secure_vault_1 = require("../core/secure-vault");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class AgentOrchestrator {
    constructor(masterWalletPassword, walletDir = './wallets') {
        this.agents = new Map();
        this.masterSession = ''; // Initialize with empty string
        this.walletDir = walletDir;
        this.masterWallet = new agentic_wallet_1.AgenticWallet(process.env.SOLANA_RPC_URL || 'https://api.devnet.solana.com', walletDir);
     ...
     */
    initializeMasterWalletSync(password) {
     ...
     */
    async initializeMasterWallet(password) {
        const masterPath = path.join(this.walletDir, 'master.json');
        try {
            if (!fs.existsSync(masterPath)) {
                console.log('📦 Creating new master wallet...');
     ...
     */
    async createAgent(agentId, initialFunding = 0.1, agentPassword) {
        ...
        try {
            ...
            const password = agentPassword || secure_vault_1.SecureVault.generateSecurePassword();
            // 1. Create wallet for this agent
            const wallet = new agentic_wallet_1.AgenticWallet(process.env.SOLANA_RPC_URL || 'https://api.devnet.solana.com', this.walletDir);
            ...
            if (initialFunding > 0) {
                try {
                    ...
                    const sig = await this.masterWallet.transferSol(address, initialFunding, this.masterSession);
            ...
            // 4. Create brain
            const brain = new decision_engine_1.DecisionEngine(wallet, sessionToken, agentId);
     ...
     */
    async runAllAgents(cycles = 3, intervalMs = 5000) {
     ...
     */
    async createAgents(count, baseFunding = 0.1) {
